...
public class AllTierListsResult
{
    /// <summary>
    ...
    /// </summary>
    public IReadOnlyCollection<ITierListDto>? TierListsData { get; init; }
    ...
    /// if provided.</returns>
    public static AllTierListsResult Success(IReadOnlyCollection<ITierListDto>? data = null) => new() { IsSuccess = true, TierListsData = data ?? null };
    ...
    public static AllTierListsResult Failure(string error, ErrorType errorType) => new() { IsSuccess = false, ErrorMessage = error, ErrorType = errorType };
}
